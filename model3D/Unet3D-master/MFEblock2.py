...
class ASPPConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, dilation):
        modules = [
            nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=dilation, dilation=dilation, stride=2, bias=False),#groups = in_channels
            nn.BatchNorm3d(out_channels),
            nn.ReLU()
        ]
...
class MFEblock(nn.Module):
    def __init__(self, in_channels, atrous_rates):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, 3, padding=1, dilation=1, bias=False, stride=2),#groups = in_channels , bias=False
            nn.BatchNorm3d(out_channels),
            nn.ReLU())
        ...
        self.SE3 = oneConv(out_channels,out_channels,1,0,1)
        self.SE4 = oneConv(out_channels,out_channels,1,0,1)
    def forward(self, x):
        # x: (B,C,H,W)

        ### 多特征提取: Multi-Features Extraction block, MFEblock
        y0 = self.layer1(x)    # 第一个分支的输入只有x: (B,C,H,W)-->(B,C,H,W)
        y1 = self.layer2(x) # 第二个分支的输入是y0和x: (B,C,H,W)-->(B,C,H,W)
        y2 = self.layer3(x) # 第三个分支的输入是y1和x: (B,C,H,W)-->(B,C,H,W)
        y3 = self.layer4(x) # 第四个分支的输入是y2和x: (B,C,H,W)-->(B,C,H,W)
